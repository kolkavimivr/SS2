...
app.all(['/survey', '/ivr', '/'], (req, res) => {
    ...
    // -------------------------------------------------------------
    // 2. בדיקת שאלות ברצף (הפרדת פתיחים באמצעות id_list_message)
    // -------------------------------------------------------------

    // שאלה 01: השמעת פתיח ראשי באופן אוטומטי + שאלה q01
    if (!params.q01) {
        return res.send('id_list_message=f-quiz_intro&read=f-q01=q01,yes,1,1,7,Digits,no,no,1.2.3');
    }

    // שאלות 02 עד 14 (קליטה ישירה ללא אישור)
    ...
    // שאלה 15: השמעת מעברון חלק ג' באופן אוטומטי + שאלה q15
    if (!params.q15) {
